...
import com.buddies.catchwo.Adapter.ReCommentAdapter;
...
public class ReCommentActivity extends AppCompatActivity {
        ...
        sendBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(TextUtils.isEmpty(messageBox.getText().toString())){
                    Toast.makeText(ReCommentActivity.this, "Please Comment On the Post", Toast.LENGTH_SHORT).show();
                }else {
                    UploadComment();
                }
            }
    ...
    private void LoadComments() {
        ...
        reference.child("ReComments").child(Pid).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                adapter = new ReCommentAdapter(ReCommentActivity.this, arrayList);
                recyclerView.setAdapter(adapter);
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(ReCommentActivity.this, error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
        ...
        DatabaseReference reference  = FirebaseDatabase.getInstance().getReference();
        reference.child("ReComments").child(Pid).child(String.valueOf(times)).setValue(hashMap).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
...
//                arrayList.clear();
                    ...
                    if (success.equals("1")) {
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            if(uid2.equals(PUid)){
                                ...
                                Glide.with(ReCommentActivity.this).load(image).into(user_img);
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(ReCommentActivity.this);
